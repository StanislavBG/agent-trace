...
function printTree(tree, parentId, depth) {
    ...
    for (const span of children) {
        ...
        if (attrs['command'])
            keyAttrs.push(chalk.yellow(`cmd=${String(attrs['command']).slice(0, 50)}`));
        if (attrs['gen_ai.system'])
            keyAttrs.push(chalk.magenta(String(attrs['gen_ai.system'])));
        if (attrs['gen_ai.request.model'])
            keyAttrs.push(chalk.yellow(String(attrs['gen_ai.request.model'])));
        if (attrs['exit_code'] != null)
            keyAttrs.push(chalk.dim(`exit=${attrs['exit_code']}`));
        if (attrs['gen_ai.usage.input_tokens'] != null)
            keyAttrs.push(chalk.dim(`in=${attrs['gen_ai.usage.input_tokens']}`));
        if (attrs['gen_ai.usage.output_tokens'] != null)
            keyAttrs.push(chalk.dim(`out=${attrs['gen_ai.usage.output_tokens']}`));
...
async function runShow(traceIdPrefix, opts) {
